...
const Sales = () => {
    ...
    const { data: inventoryItems = [], isLoading: isInventoryLoading } = useQuery({
        queryKey: ['inventory', profile?.shop_id],
        queryFn: async () => {
    ...
    const { data: salesHistory = [] } = useQuery({
        ...
        queryFn: async () => {
            if (!user) return [];
            const selectStatement = isShopAdmin ? '*, profiles(email)' : '*';
            let query = supabase.from('sales').select(selectStatement).order('created_at', { ascending: false });
            if (role !== 'shop_admin') query = query.eq('shop_id', profile?.shop_id);
    ...
    // --- ✅ FIXED: Mutation uses direct inventory update instead of broken RPC ---
    const processSaleMutation = useMutation({
        mutationFn: async (billData: BillData) => {
            ...
            // Step 2: Decrement inventory quantities directly
            for (const item of billData.items) {
                ...
                if (fetchError) {
                    console.warn(`Could not fetch stock for ${item.item_name}: ${fetchError.message}`);
                    continue;
                }

                const newQuantity = Math.max(0, (current?.quantity ?? 0) - item.cart_quantity);
                ...
                if (stockError) {
                    console.warn(`Could not update stock for ${item.item_name}: ${stockError.message}`);
                }
            }
        },
        onSuccess: () => {
    ...
    const completeSale = async (paymentMethod: 'cash' | 'online') => {
        ...
        if (isOffline) {
            ...
            try {
                ...
                setDialogs({ payment: false, success: false, history: false });
            }
        }
    };
            ...
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                <header className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        {pendingSaleCount > 0 && !isSyncing && (
                            <Badge variant="outline" className="flex items-center gap-1.5 py-1 px-2 border-amber-400 text-amber-600">
                                <History className="h-4 w-4" />{pendingSaleCount} pending
                            </Badge>
                        )}
                        {isOffline && (
                            <Badge variant="destructive" className="flex items-center gap-1.5 py-1 px-2 animate-pulse">
                                <ServerOff className="h-4 w-4" />Offline Mode
                            </Badge>
                        )}
                        {isShopAdmin && (
                            <Badge variant="default" className="flex items-center gap-1.5 py-1 px-2 bg-indigo-600 hover:bg-indigo-700">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        {filteredInventoryItems.map(item => {
                                            ...
                                            return (
                                                <Card
                                                    key={item.id}
                                                    className={`relative overflow-hidden transition-all group ${isUnavailable
                                                    ...
                                                    {isUnavailable && (
                                                        <div className="absolute inset-0 bg-slate-500/40 flex items-center justify-center z-10">
                                                            <Badge variant="destructive" className="text-sm">
                                                                {item.quantity <= 0 ? 'Out of Stock' : 'Unavailable'}
                                                            </Badge>
                                                        </div>
                                                    )}
                                                    <CardContent className="p-3 flex flex-col justify-between h-full">
                                                        <div>
                                                            <h3 className={`font-semibold text-sm truncate ${!isUnavailable ? 'group-hover:text-primary' : ''}`}>
                                                                {item.item_name}
                                                            </h3>
                                                            <p className="font-bold text-base my-1">{formatCurrency(item.unit_price)}</p>
                                                        </div>
                                                        <Badge
                                                            variant={item.quantity > 10 ? 'outline' : 'destructive'}
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab}>
                                ...
                                <TabsContent value="cart" className="p-4">
                                        ...
                                        {cart.length === 0 ? (
                                            ...
                                            <div key={item.id} className="flex items-center gap-4">
                                                ...
                                                <div className="flex items-center gap-1 border rounded-md">
                                                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => updateCartQuantity(item.id, item.cart_quantity - 1)}>
                                    ...
                                    {cart.length > 0 && (
                                        <div className="mt-4 border-t pt-4 space-y-2">
                                            ...
                                            <Button onClick={proceedToPayment} className="w-full mt-4">
                                ...
                                <TabsContent value="customer" className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
                                    <div>
                                        ...
                                        <Input id="c-phone" value={customer.phone} onChange={e => setCustomer({ ...customer, phone: e.target.value })} />
                                    </div>
                                    <div>
                                        <Label htmlFor="c-name">Name (Optional)</Label>
                                        <Input id="c-name" value={customer.name} onChange={e => setCustomer({ ...customer, name: e.target.value })} />
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <Label htmlFor="notes">Notes for Invoice</Label>
                        ...
                        <Tabs defaultValue="upi" className="w-full">
                            ...
                            <TabsContent value="upi" className="text-center p-4 space-y-4">
                                ...
                                <div className="p-4 bg-white inline-block rounded-lg border">
                                    <QRCodeSVG
                                        value={`upi://pay?pa=${defaultCompanyInfo.upi_id}&pn=${encodeURIComponent(defaultCompanyInfo.name)}&am=${cartTotals.finalAmount.toFixed(2)}&tn=${paymentBillId}`}
                                        size={180}
                                    />
                                </div>
                                <Button
                                ...
                                >
                                    {(processSaleMutation.isPending && !isOffline)
                                        ? <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                        : <CheckCircle className="h-4 w-4 mr-2" />
                                    }
                                    {isOffline ? 'Save Offline' : 'Confirm Payment'}
                                </Button>
                            </TabsContent>
                            <TabsContent value="cash" className="p-4 space-y-4">
                                <div className="text-center p-6 border rounded-lg bg-slate-50">
                                    ...
                                    <p className="text-4xl font-bold text-primary">{formatCurrency(cartTotals.finalAmount)}</p>
                                </div>
                                <Button
                                    className="w-full"
                                ...
                                >
                                    ...
                                    {isOffline ? 'Save Offline' : 'Confirm Cash Payment'}
                                </Button>
                            </TabsContent>
                        </Tabs>
                    </DialogContent>
                ...
                <Dialog open={dialogs.success} onOpenChange={(isOpen) => setDialogs({ ...dialogs, success: isOpen })}>
                    <DialogContent className="max-w-md">
                        ...
                        <div className="py-6 space-y-3">
                            <Button className="w-full" variant="outline" onClick={() => completedBill && generatePdfInvoice(completedBill)}>
                                <Download className="h-4 w-4 mr-2" />Download PDF Invoice
                            </Button>
                        </div>
                        <DialogFooter>
                            <Button className="w-full" onClick={() => { setDialogs({ ...dialogs, success: false }); resetSale(); }}>
                ...
                <Dialog open={dialogs.history} onOpenChange={(isOpen) => setDialogs({ ...dialogs, history: isOpen })}>
                    <DialogContent className="max-w-2xl">
                        <DialogHeader>
                            ...
                            <DialogDescription>Invoice ID: {selectedHistorySale?.bill_data?.billId}</DialogDescription>
                        </DialogHeader>
                        {selectedHistorySale?.bill_data && (
                            <div className="py-4 max-h-[70vh] overflow-y-auto">
                                <Table>
                                    ...
                                    <TableBody>
                                        {selectedHistorySale.bill_data.items.map((item: BillItem, index: number) => (
                                            <TableRow key={index}>
                                ...
                                <div className="flex justify-end mt-4 pt-4 border-t">
                                    <div className="w-64">
                                        <div className="flex justify-between font-bold text-base">
                                            ...
                                            <span>{formatCurrency(selectedHistorySale.total_amount)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <DialogFooter>
                            <Button variant="outline" onClick={() => selectedHistorySale?.bill_data && generatePdfInvoice(selectedHistorySale.bill_data)}>
                                Download Again
                            </Button>
                            <Button onClick={() => setDialogs({ ...dialogs, history: false })}>Close</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </div>
...
const CustomerHistoryPanel = ({
    ...
    const customerGroups = useMemo(() => {
        ...
        const sortedGroups = Array.from(groups.values()).sort(
            (a, b) => new Date(b.sales[0].created_at).getTime() - new Date(a.sales[0].created_at).getTime()
        );
        if (!historySearch) return sortedGroups;
        return sortedGroups.filter(
            group =>
                group.name.toLowerCase().includes(historySearch.toLowerCase()) ||
                group.sales[0].customer_phone.includes(historySearch)
        );
    ...
    if (selectedPhone) {
        ...
        return (
            <div className="p-4">
                <Button variant="ghost" onClick={() => setSelectedPhone(null)} className="mb-4 -ml-4">
                ...
                <div className="space-y-3 max-h-[50vh] overflow-y-auto">
                    {customerData?.sales.map(sale => (
                        <div
                            key={sale.id}
                            className="text-sm p-3 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer"
                            onClick={() => onSaleSelect(sale)}
                        >
                            <div className="flex justify-between font-semibold">
                                <span>{sale.bill_data?.billId || 'INV-LEGACY'}</span>
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground mt-1 space-x-2">
                                ...
                                {isAdmin && (
                                    <span className="truncate" title={sale.profiles?.email || 'Unknown'}>
                                        By: {sale.profiles?.email?.split('@')[0] || 'N/A'}
                                    </span>
                                )}
        ...
        <div className="p-4 space-y-3">
            ...
            <div className="max-h-[52vh] overflow-y-auto space-y-3">
                {customerGroups.length === 0 ? (
                    <p className="text-center text-sm text-muted-foreground py-10">No customer history found.</p>
                ) : customerGroups.map(group => (
                    <div
                        key={group.sales[0].customer_phone}
                        className="p-3 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer"
                        onClick={() => setSelectedPhone(group.sales[0].customer_phone)}
                    >
                        <div className="flex justify-between font-semibold">
                            <span>{group.name || 'Walk-in'}</span>
                            <Badge variant="secondary">{group.sales.length} orders</Badge>
                        </div>
