...
import {
  ShoppingCart, Sparkles, TrendingUp, Package,
  Star, Clock, Flame, ChevronRight, MessageSquare,
...
const CustomerDashboard = () => {
  ...
  // ── Fetch past orders (shop-isolated, correct column names) ──
  ...
  const stats = useMemo(() => {
    ...
    // FIX: use cart_quantity (correct JSONB field) and total_price
    const itemCounts: Record<string, { count: number; price: number }> = {};
    pastOrders.forEach(order => {
      parseItems(order.items).forEach((item: any) => {
        const name = item.item_name;
        if (!name) return;
        if (!itemCounts[name]) itemCounts[name] = { count: 0, price: item.unit_price || 0 };
        // cart_quantity is the correct field from your JSONB schema
        itemCounts[name].count += Number(item.cart_quantity) || 1;
      });
    });
  ...
  // FIX: trending = most quantity (popular stock), not highest price
  const trending = useMemo(() =>
    [...inventory].sort((a, b) => b.quantity - a.quantity).slice(0, 6),
    [inventory]
  );

  // FIX: deals = items with low stock (urgency), quantity 1–20
  const freshDeals = useMemo(() =>
    inventory.filter(i => i.quantity <= 20 && i.quantity > 0).slice(0, 6),
    [inventory]
  );
      ...
      <div className="container mx-auto px-4 py-6 max-w-6xl">
            ...
            <div>
              ...
              <p className="text-indigo-100 text-sm max-w-md opacity-90">
                {stats.totalOrders === 0
                  ...
                  : `You've made ${stats.totalOrders} orders with us. Here are your personalized picks.`}
              </p>
              {stats.topCategory && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <Star className="h-4 w-4 text-yellow-500" /> Loyalty Status
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <RefreshCw className="h-4 w-4 text-green-500" /> Buy Again
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {reorderItems.length > 0 ? reorderItems.map(item => (
                ...
                <p className="text-xs text-center text-gray-400 py-4">Place an order to see items here.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm bg-indigo-50 border-l-4 border-indigo-500">
            <CardContent className="p-5">
              ...
              <p className="text-xs text-indigo-600 leading-relaxed mb-4">
                "I can help you find products, suggest recipes, or check which items are currently freshest."
              </p>
              <Button size="sm" className="w-full bg-indigo-600 hover:bg-indigo-700" onClick={() => navigate('/ai-assistant')}>
                Chat Now
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex bg-gray-100 p-1 rounded-lg">
              {(['recommended', 'deals', 'trending'] as const).map((sec) => (
                <button
                  key={sec}
          ...
          {displayItems.length === 0 ? (
            <p className="text-center py-10 text-gray-400 text-sm">No items available in this section.</p>
          ) : (
        ...
        {pastOrders.length > 0 && (
          <Card className="border-0 shadow-sm mb-8">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Clock className="h-4 w-4 text-gray-500" /> Recent Purchase History
              </CardTitle>
            </CardHeader>
                ...
                return (
                  <div key={order.id} className="flex items-center justify-between p-4 border rounded-xl hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold">{orderItems.length} item{orderItems.length !== 1 ? 's' : ''} purchased</p>
                        <p className="text-[10px] text-gray-400">
                          {format(parseISO(order.created_at), 'dd MMM yyyy, hh:mm a')}
                        </p>
                      </div>
                    </div>
...
const ProductCard = ({ item, onAdd }: { item: InventoryItem; onAdd: (item: InventoryItem) => void }) => (
  <Card className="overflow-hidden border-0 shadow-sm group h-full">
    <div className="h-32 bg-gray-100 relative flex items-center justify-center text-[10px] font-bold text-gray-400 uppercase">
      ...
      {item.quantity <= 10 && (
        <span className="absolute top-2 right-2 bg-red-500 text-white px-2 py-0.5 rounded text-[8px]">Low Stock</span>
      )}
    </div>
    <CardContent className="p-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          size="sm"
          variant="outline"
          className="h-7 w-7 p-0 rounded-full border-indigo-200 text-indigo-600"
          onClick={() => onAdd(item)}
        >
          +
        </Button>
