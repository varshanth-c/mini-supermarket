...
import {
  Plus, ShoppingCart, Download, Mail, Check, Loader2, Minus, QrCode, CreditCard, Store, Sparkles,
  CheckCircle, Search, ShoppingBag,
} from 'lucide-react';
...
const CustomerPOSPage = () => {
  ...
  // FIX: get profile (has shop_id) as well as user
  ...
  const [companyInfo] = useState<CompanyInfo>(() => {
    ...
    return saved ? JSON.parse(saved) : {
      name: 'Your Company', address: '123 Business St',
      phone: '9876543210', email: 'contact@company.com',
  ...
  // User profile → pre-fill customer
  ...
  // FIX: fetch inventory scoped to the current user's shop_id — no admin lookup needed
  const { data: inventoryItems = [], isLoading: isInventoryLoading } = useQuery({
    queryKey: ['shopInventory', profile?.shop_id],
    queryFn: async () => {
      if (!profile?.shop_id) return [];
      const { data, error } = await supabase
        .from('inventory')
        .select('id, item_name, category, quantity, unit_price, barcode, image_url')
        .eq('shop_id', profile.shop_id)
        .gt('quantity', 0)         // only show in-stock items to customer
        .order('item_name');
  ...
  // Past orders — scoped to user + shop
  const { data: pastOrders = [] } = useQuery({
    queryKey: ['pastOrders', user?.id, profile?.shop_id],
    queryFn: async () => {
  ...
  const processSaleMutation = useMutation({
    mutationFn: async (payload: { cartItems: any[]; customer: any; billData: any }) => {
      const { data, error } = await supabase.functions.invoke('process-sale', { body: payload });
      if (error) throw new Error(`Transaction failed: ${error.message}`);
      return data;
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['shopInventory', profile?.shop_id] });
      queryClient.invalidateQueries({ queryKey: ['pastOrders', user?.id, profile?.shop_id] });
      toast({ title: 'Order Placed!', description: 'Your payment was successful.', className: 'bg-green-100 border-green-400' });
  ...
  const sendEmailMutation = useMutation({
    ...
    onSuccess: () => { toast({ title: 'Email Sent!', description: 'The invoice has been sent.' }); },
    onError: (error: Error) => { toast({ title: 'Email Failed', description: error.message, variant: 'destructive' }); },
  });

  // FIX: BillItem now uses cart_quantity and total_price to match JSONB schema
  const cartTotals = useMemo(() => {
    const subtotal = cart.reduce((acc, item) => acc + item.cart_quantity * item.unit_price, 0);
    const billItems: BillItem[] = cart.map(item => ({
      ...
      final_amount: item.cart_quantity * item.unit_price,
    }));
  ...
  const proceedToPayment = () => {
    if (cart.length === 0) {
      toast({ title: 'Your cart is empty', description: 'Please add items before proceeding.', variant: 'destructive' });
      return;
    }
    if (!customer.name || !customer.phone) {
      toast({ title: 'Details missing', description: 'Please ensure your name and phone are in your profile.', variant: 'destructive' });
      return;
    }
  ...
  const addToCart = (item: InventoryItem) => {
    ...
    if (existing) {
      updateCartQuantity(item.id, existing.cart_quantity + 1);
    } else {
      if (item.quantity > 0) {
  ...
  const updateCartQuantity = (itemId: string, newQuantity: number) => {
    ...
    } else if (newQuantity > itemInStock.quantity) {
      toast({ title: 'Stock limit reached', description: `Only ${itemInStock.quantity} available.`, variant: 'destructive' });
    } else {
  ...
  const generatePdfInvoice = (bill: BillData | null, outputType: 'save' | 'base64' = 'save') => {
    ...
    doc.text(`Grand Total: ₹${bill.finalAmount.toFixed(2)}`, 196, finalY + 15, { align: 'right' });
    if (outputType === 'save') { doc.save(`invoice-${bill.billId}.pdf`); return null; }
    return doc.output('datauristring').split(',')[1];
  };

  const handleSendEmail = () => {
    if (!completedBill?.customer.email) {
      toast({ title: 'Customer email not found.', variant: 'destructive' }); return;
    }
    const pdfBase64 = generatePdfInvoice(completedBill, 'base64');
    if (pdfBase64) {
      sendEmailMutation.mutate({
        to: completedBill.customer.email,
        subject: `Invoice from ${completedBill.companyInfo.name}`,
        html: `<p>Hi ${completedBill.customer.name},</p><p>Thank you for your purchase! Your invoice is attached.</p>`,
        pdfBase64,
        pdfName: `invoice-${completedBill.billId}.pdf`,
      });
    }
  };

  // FIX: show loading only when we have no shop_id yet (profile not loaded), or inventory is loading
  if (!profile || isProfileLoading || isInventoryLoading) {
    return (
  ...
  // FIX: show clear error if no shop is associated
  if (!profile.shop_id) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <p className="text-gray-500">No shop associated with your account. Please contact support.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {cart.length === 0 ? (
          ...
          <motion.div layout key={item.id} className="flex items-center gap-4">
            <img
            ...
            <p className="font-semibold">₹{(item.cart_quantity * item.unit_price).toFixed(2)}</p>
          </motion.div>
        ))}
      </div>
      {cart.length > 0 && (
        <div className="p-4 border-t bg-white dark:bg-gray-950">
          ...
          <Button onClick={proceedToPayment} size="lg" className="w-full bg-blue-600 hover:bg-blue-700 text-white">
          ...
          <header className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-3">
                <Store className="h-8 w-8 text-primary" /> Place Your Order
              </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2">
                ...
                {filteredInventoryItems.length === 0 && (
                  <p className="text-center py-10 text-gray-500">No products match your search.</p>
                )}
              </div>
          ...
          <TabsContent value="history">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your recent completed orders.</CardDescription>
              </CardHeader>
      ...
      <Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Choose a payment method for ₹{cartTotals.finalAmount.toFixed(2)}</DialogDescription>
          </DialogHeader>
          <Tabs defaultValue="online" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="online">UPI / QR</TabsTrigger>
              <TabsTrigger value="cash">Cash</TabsTrigger>
            </TabsList>
            <TabsContent value="online" className="text-center p-4 space-y-4">
              <p>Scan the QR code to pay using any UPI app.</p>
              <div className="p-4 bg-white inline-block rounded-lg border-2 border-dashed">
                <QRCodeSVG
              ...
              <p className="text-xs text-gray-500">UPI ID: {companyInfo.upi_id}</p>
              <Button className="w-full bg-green-600 hover:bg-green-700" onClick={() => completeSale('online')} disabled={processSaleMutation.isPending}>
                {processSaleMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4 mr-2" />}
                Confirm Payment
              </Button>
            </TabsContent>
            <TabsContent value="cash" className="p-4 space-y-4">
              <div className="text-center p-6 border rounded-lg bg-gray-50 dark:bg-gray-800/50">
                <p className="text-gray-600 dark:text-gray-400">Please pay at the counter.</p>
                <p className="text-4xl font-bold text-primary">₹{cartTotals.finalAmount.toFixed(2)}</p>
              </div>
              <Button className="w-full bg-green-600 hover:bg-green-700" onClick={() => completeSale('cash')} disabled={processSaleMutation.isPending}>
                {processSaleMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="h-4 w-4 mr-2" />}
      ...
      <Dialog open={showSaleSuccessDialog} onOpenChange={setShowSaleSuccessDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader className="text-center">
            ...
            <DialogTitle className="text-2xl mt-4">Thank You For Your Order!</DialogTitle>
            <DialogDescription>Your payment was successful and your order has been recorded.</DialogDescription>
          </DialogHeader>
          <div className="py-6 space-y-3">
            <Button className="w-full" variant="outline" onClick={() => generatePdfInvoice(completedBill, 'save')}>
              <Download className="h-4 w-4 mr-2" /> Download PDF Invoice
            </Button>
            {completedBill?.customer.email && (
              <Button className="w-full" variant="outline" onClick={handleSendEmail} disabled={sendEmailMutation.isPending}>
          ...
          <DialogFooter>
            <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => { setShowSaleSuccessDialog(false); resetSale(); }}>
              <Plus className="h-4 w-4 mr-2" /> Start New Order
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
